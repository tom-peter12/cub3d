...
/*   Updated: 2023/12/12 18:06:45 by tpetros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_line_height(t_game *game)
{
	if (game->ray.dda.side == 0)
		game->ray.dda.wall_dist = (game->ray.dda.map.x - game->fps.pos.x
		+ (1 - game->ray.dda.step.x) / 2) / game->ray.ray_vec.x;
	else if (game->ray.dda.side == 1)
		game->ray.dda.wall_dist = (game->ray.dda.map.y - game->fps.pos.y 
		+ (1 - game->ray.dda.step.y) / 2) / game->ray.ray_vec.y;
...
void	pre_dda(t_game *game)
{
	...
	if (game->ray.ray_vec.x < 0)
	{
		...
		game->ray.dda.side_dist.x = (game->fps.pos.x - game->ray.dda.map.x) 
...
void dda(t_game *game)
{
    ...
    while (game->ray.dda.hit == 0)
	{
		...
		if (game->parse.map->tab[(int)game->ray.dda.map.y % game->parse.map->height][(int)game->ray.dda.map.x % game->parse.map->width] == '1')
			game->ray.dda.hit = 1;
    }
}
